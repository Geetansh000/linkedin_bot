    ...
    for path in paths:
        ...
        try:
            if not os.path.exists(path):
                # exist_ok=True avoids race condition
...
def find_default_profile_directory() -> str | None:
    ...
    # Linux
        ...
        paths = [
            ...
            str(home / ".var" / "app" / "com.google.Chrome" /
                "data" / ".config" / "google-chrome"),
        ]
...
def get_log_path():
    ...
    try:
        ...
        critical_error_log(
            "Failed getting log path! So assigning default logs path: './logs/log.txt'", e)
        return "logs/log.txt"
...
def print_lg(*msgs: str | dict, end: str = "\n", pretty: bool = False, flush: bool = False, from_critical: bool = False) -> None:
    ...
    try:
        ...
        if not from_critical:
            critical_error_log(
                "Log.txt is open or is occupied by another program!", e)
# >
...
def calculate_date_posted(time_string: str) -> datetime | None | ValueError:
    ...
    * 1 year ago
    ...
    match = re.search(
        r'(\d+)\s+(second|minute|hour|day|week|month|year)s?\s+ago', time_string, re.IGNORECASE)
